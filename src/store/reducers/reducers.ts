...
import {ProfileState, ADD_PROFILE, ProfileActionTypes, CHECK_PASS, LOG_OUT} from '../types/types'
...
export function profileReducer(state = initialState, action: ProfileActionTypes) : ProfileState {
    switch(action.type){
        ...
        case CHECK_PASS:
            ...
            let index = 0;
            let uName = state.profiles.filter(profile => profile.name === action.payload[0]);
            console.log(uName);
        ...
        case LOG_OUT:
            let uNameLog = state.profiles.filter(profile => profile.name === action.payload.name);
            let indexLog = 0;
            state.profiles.forEach((profile, i) => {
            ...
            return{
                ...state,
                loggedIn: false,
                profiles: [...state.profiles.slice(0, indexLog), {...state.profiles[indexLog], loggedIn: false}, ...state.profiles.slice(indexLog+1)]
            }
        default:
            return state;
    }
